...
from localshop.apps.permissions.models import CIDR, Hostname
...
def credentials_required(view_func):
    ...
    def decorator(request, *args, **kwargs):
        ...
        for hostname in Hostname.objects.all():
            if hostname.is_matching(ip_addr):
                is_hostname_verified = True
                break

        if not CIDR.objects.has_access(ip_addr, with_credentials=True) and not is_hostname_verified:
